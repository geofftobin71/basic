...
class Display():
    ...
    def __init__(self, width, height, ctx):
        ...
        self.screen_prog = self.ctx.program(
            ...
            void main()
            {
                uint cell = texelFetch(char_sampler,
                    ivec2(gl_FragCoord.xy) >> 3, 0).r;

                uint cell_x = (cell & 0x0FU) << 3;
                uint cell_y = (cell & 0xF0U) >> 1;

                uint pixel_x = uint(gl_FragCoord.x) & 0x07U;
                uint pixel_y = uint(gl_FragCoord.y) & 0x07U;
