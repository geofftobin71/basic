...
class Interpreter(NodeVisitor):
    ...
    def visit_AssignNode(self, node):
        ...
        if var is None and len(node.left.idxs) > 0:
            ...
            self.global_scope[var_name] = self.init_list(dims, val)
            var = self.global_scope.get(var_name)
        ...
        if var is not None and type(var) is list:
            ...
            self.set_list(self.global_scope[var_name], value, idxs, tokens)
        else:
    ...
    def visit_VarNode(self, node):
        ...
        if type(var) is list:
            ...
            value = self.get_list(var, idxs, tokens)
        else:
            value = var
    ...
    def init_list(self, dims, val):
        ...
        return [self.init_list(dims[1:], val=val) for _ in range(dims[0])]

    def get_dims(self, data):
        if not isinstance(data, list):
            return 0
        return self.get_dims(data[0]) + 1

    def set_list(self, data, value, idxs, tokens):
        if len(idxs) == 0:
            raise ValueError("Requires at least 1 dimension")
        ...
        if len(idxs) == 1:
            ...
            self.set_list(data[idxs[0]], value, idxs[1:], tokens[1:])

    def get_list(self, data, idxs, tokens):
        if len(idxs) == 0:
            raise ValueError("Requires at least 1 dimension")
        ...
        if len(idxs) == 1:
            ...
            return self.get_list(data[idxs[0]], idxs[1:], tokens[1:])
