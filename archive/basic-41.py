...
class Interpreter(NodeVisitor):
    ...
    def visit_StyleNode(self, node):
        ...
        op = node.op.type
        if op == TokenType.BG:
            self.window.bg_color = value
            # print(f"\033[48:5:{value}m", end="")
        elif op == TokenType.FG:
            self.window.fg_color = value
            # print(f"\033[38:5:{value}m", end="")
        elif op == TokenType.INVERSE:
            if value:
                tmp = self.window.fg_color
                self.window.fg_color = self.window.bg_color
                self.window.bg_color = tmp
                # print("\033[7m", end="")
            else:
                tmp = self.window.fg_color
                self.window.fg_color = self.window.bg_color
    ...
    def visit_CursorNode(self, node):
        ...
        self.window.print_cursor_x = x
        self.window.print_cursor_y = y
