...
#
# Math functions
# TODO : Check function arg types, test assigning result of function types
#
...
class Parser():
    ...
    def parse_value(self):
        ...
        if token.type == TokenType.NUMBER:
            ...
            self._eat(TokenType.RPAREN)
        elif token.type in (TokenType.ABS,
                            TokenType.ASC,
                            TokenType.CHR):
...
class Interpreter(NodeVisitor):
    ...
    def visit_AssignNode(self, node):
        ...
        if var_name.endswith('$') and type(value) is not str:
            message = f'Type mismatch at {var_name}'
            lineno = node.left.token.lineno
            column = node.left.token.column
    ...
    def visit_VarNode(self, node):
        ...
        if value is None:
            message = f"Identifier '{var_name}' not found"
    ...
    def visit_BinOpNode(self, node):
        if node.op.type == TokenType.PLUS:
            ...
            return 1.0 if self.visit(node.left) == \
                            self.visit(node.right) else 0.0
            ...
            return 1.0 if (self.visit(node.left) != 0.0) and \
                        (self.visit(node.right) != 0.0) else 0.0
        elif node.op.type == TokenType.OR:
            return 1.0 if (self.visit(node.left) != 0.0) or \
                        (self.visit(node.right) != 0.0) else 0.0
    ...
    def visit_UnaryOpNode(self, node):
        ...
        if op == TokenType.PLUS:
            ...
            return 1.0 if not (self.visit(node.expr) != 0.0) else 0.0
        elif op == TokenType.ABS:
            return abs(self.visit(node.expr))
        elif op == TokenType.ASC:
            return float(ord(str(self.visit(node.expr))[0]))
        elif op == TokenType.CHR:
            return chr(int(self.visit(node.expr)))
