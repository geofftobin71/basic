...
class Interpreter(NodeVisitor):
    ...
    def type_mismatch_error(self, op):
        message = f"Type mismatch at '{op.value}'"
        lineno = op.lineno
        column = op.column
    ...
    def visit_AssignNode(self, node):
        ...
        if var_name.endswith('$') and type(value) is not str:
            self.type_mismatch_error(node.op)
    ...
    def visit_BinOpNode(self, node):
        ...
        if node.op.type == TokenType.PLUS:
            ...
            if type(left_value) is str:
                ...
                return 1.0 if (left_value != 0.0) and \
                    (right_value != 0.0) else 0.0
        elif node.op.type == TokenType.OR:
            if type(left_value) is str:
                self.type_mismatch_error(node.op)
            else:
                return 1.0 if (left_value != 0.0) or \
                    (right_value != 0.0) else 0.0
    ...
    def visit_UnaryOpNode(self, node):
        ...
        if op == TokenType.PLUS:
            ...
            if type(value) is not str:
                ...
                message = f"Empty string at '{node.op.value}'"
                lineno = node.op.lineno
                column = node.op.column
                raise RuntimeError(message, lineno, column)
            else:
    ...
    def visit_GoToNode(self, node):
        ...
        if type(line_number) is str:
            self.type_mismatch_error(node.expr.token)
