...
class Display():
    ...
    def __init__(self, width, height, ctx):
        ...
        for i in range(len(self.char_memory) // 3):
            ...
            self.char_memory[i * 3 + 1] = random.randint(0, 255)
            self.char_memory[i * 3 + 2] = random.randint(0, 255)
        ...
        self.colors = np.zeros(256 * 3, dtype='f4')
        for i in range(256):
            self.colors[i * 3 + 0] = i / 255.0
            self.colors[i * 3 + 1] = i / 255.0
            self.colors[i * 3 + 2] = i / 255.0

        self.colors_ubo = self.ctx.buffer(reserve=256 * 3 * 4)
        self.colors_ubo.write(self.colors.tobytes())
        self.colors_ubo.bind_to_uniform_block(1)
        ...
        self.screen_prog['Colors'].binding = 1
