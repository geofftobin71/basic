...
def set_list(lst, value, idxs, tokens):
    if len(idxs) == 0:
        raise ValueError("Requires at least 1 dimension")

    if idxs[0] < 0 or idxs[0] >= len(lst):
        out_of_range_error(tokens[0])

    if len(idxs) == 1:
        lst[idxs[0]] = value
    else:
        set_list(lst[idxs[0]], value, idxs[1:], tokens[1:])


def get_list(lst, idxs, tokens):
    if len(idxs) == 0:
        raise ValueError("Requires at least 1 dimension")
    ...
    if len(idxs) == 1:
        ...
        return get_list(lst[idxs[0]], idxs[1:], tokens[1:])


def get_dims(lst):
    if not isinstance(lst, list):
        return 0
    return get_dims(lst[0]) + 1
...
def data_not_found_error(token):
    message = f"Data not found at line {int(token.value)}"
    lineno = token.lineno
    column = token.column
    raise RuntimeError(message, lineno, column)
...
class Parser():
    ...
    def parse_program(self):
        ...
        while self.current_token.type != TokenType.EOF:
            ...
            self.line_numbers[line_number] = len(root.nodes)
            for node in nodes:
    ...
    def parse_statement(self):
        ...
        if self.current_token.type == TokenType.ID:
        ...
        elif self.current_token.type == TokenType.NORMAL:
            node = self.parse_normal_statement()
        elif self.current_token.type == TokenType.INPUT:
            node = self.parse_input_statement()
        elif self.current_token.type == TokenType.PRINT:
            node = self.parse_print_statement()
        elif (self.current_token.type == TokenType.BG
              or self.current_token.type == TokenType.BLINK
              or self.current_token.type == TokenType.BOLD
              or self.current_token.type == TokenType.FAINT
              or self.current_token.type == TokenType.FG
              or self.current_token.type == TokenType.INVERSE
              or self.current_token.type == TokenType.ITALIC
              or self.current_token.type == TokenType.UNDERLINE):
            node = self.parse_style_statement()
        ...
        elif self.current_token.type == TokenType.CURSOR:
            node = self.parse_cursor_statement()
            ...
            node = self.parse_empty_statement()
    ...
    def parse_normal_statement(self):
        self.eat(TokenType.NORMAL)
        return NormalNode()
    ...
    def parse_style_statement(self):
        """
        style_statement : BG expr
                        | BLINK expr
                        | BOLD expr
                        | FAINT expr
                        | FG expr
        ...
        self.eat(token.type)
        return StyleNode(token, self.parse_expr())
    ...
    def parse_cursor_statement(self):
        """
        cursor_statement : CURSOR expr COMMA expr
        """
    ...
    def parse_data_statement(self):
        ...
        self.data_line_numbers[self.current_line_number] = len(
            self.global_data)
    ...
    def parse_restore_statement(self):
        ...
        if (self.current_token.type == TokenType.COLON
            ...
            token = Token(TokenType.NUMBER, 0, self.current_token.lineno,
                          self.current_token.column)
            lineno = NumNode(token)
        else:
            lineno = self.parse_expr()

        return RestoreNode(lineno)
...
class Interpreter(NodeVisitor):
    ...
    def __init__(self, parser, window):
        self.tree = parser.program
        self.window = window
        self.next_node = 0
        self.return_stack = []
        self.for_stack = []
        self.line_numbers = parser.line_numbers
        self.data_line_numbers = parser.data_line_numbers
        self.global_scope = {}
        self.global_data = parser.global_data
        self.global_data_ptr = 0
        self.current_node = 0

    def visit_ProgramNode(self, node):
        self.next_node = self.current_node + 1
        self.visit(node.nodes[self.current_node])
        self.current_node = self.next_node
        return not (self.current_node is None
                    or self.current_node >= len(node.nodes))

    def visit_ClsNode(self, node):
        # print("\033[2J\033[;H", end="")
        self.window.cls()

    def visit_NormalNode(self, node):
        print("\033[0m", end="")
    ...
    def visit_StyleNode(self, node):
        ...
        op = node.op.type
        if op == TokenType.BG:
            print(f"\033[48:5:{value}m", end="")
        elif op == TokenType.BLINK:
            if value:
                print("\033[5m", end="")
            else:
                print("\033[25m", end="")
        elif op == TokenType.BOLD:
            if value:
                print("\033[1m", end="")
            else:
                print("\033[22m", end="")
        elif op == TokenType.FAINT:
            if value:
                print("\033[2m", end="")
            else:
                print("\033[22m", end="")
        elif op == TokenType.FG:
            print(f"\033[38:5:{value}m", end="")
        elif op == TokenType.INVERSE:
            if value:
                print("\033[7m", end="")
            else:
                print("\033[27m", end="")
        elif op == TokenType.ITALIC:
            if value:
                print("\033[3m", end="")
            else:
                print("\033[23m", end="")
        elif op == TokenType.UNDERLINE:
            if value:
                print("\033[4m", end="")
            else:
                print("\033[24m", end="")
    ...
    def visit_InputNode(self, node):
        if self.window.input_ready():
            self.window.start_input(node.prompt)
            self.next_node = self.current_node
            return

        if self.window.input_editing():
            self.next_node = self.current_node
            return

        if self.window.input_done():
            self.next_node = self.current_node + 1
            # values = input(f"{node.prompt}? ").split(',')
            values = self.window.input_buffer.decode().split(',')
    ...
    def visit_UnaryOpNode(self, node):
        ...
        if op == TokenType.PLUS:
            ...
            if type(value) is not str:
                ...
                s = ""
                p = 0
                while (p < len(value) and not value[p].isdigit()
    ...
    def visit_CursorNode(self, node):
        x = self.visit(node.x)
        if type(x) is str:
            type_mismatch_error(node.expr.token)
        x = int(x) + 1
    ...
    def visit_RestoreNode(self, node):
        ...
        if line_number > 0:
            self.global_data_ptr = self.data_line_numbers.get(int(line_number))
            if self.global_data_ptr is None:
                data_not_found_error(node.expr.token)
        else:
            self.global_data_ptr = 0
...
def main():
    ...
    with open(args.filename, 'r') as file:
        ...
        window = Window(480, 360)
        window.cls()
        interpreter = Interpreter(parser, window)
