...
class Window(pyglet.window.Window):
    ...
    def __init__(self, width, height):
        ...
        self.input_cursor_start_x = self.dsp.cursor_x
        self.input_cursor_start_y = self.dsp.cursor_y
    ...
    def on_text(self, text):
        self.set_bg(self.input_cursor_start_x + self.input_buffer_cursor,
                    self.input_cursor_start_y, 0)
        self.input_buffer.insert(self.input_buffer_cursor, ord(text))
        self.input_buffer_cursor += 1
        self.dsp.cursor_x = self.input_cursor_start_x
        self.dsp.cursor_y = self.input_cursor_start_y
        self.dsp.print(f"{self.input_buffer.decode()} ")
        self.set_bg(self.input_cursor_start_x + self.input_buffer_cursor,
                    self.input_cursor_start_y, 8)
        self.dsp.update_chars()
    ...
    def on_text_motion(self, motion):
        ...
        if (motion == key.MOTION_BACKSPACE and self.input_buffer_cursor > 0):
            del self.input_buffer[self.input_buffer_cursor - 1:
                                  self.input_buffer_cursor]
            self.input_buffer_cursor -= 1
    ...
    def set_char(self, x, y, c):
        self.dsp.set_char(x, y, c)

    def set_fg(self, x, y, fg):
        self.dsp.set_fg(x, y, fg)

    def set_bg(self, x, y, bg):
        self.dsp.set_bg(x, y, bg)

    def get_char(self, x, y):
        return self.dsp.get_char(x, y)

    def get_fg(self, x, y):
        return self.dsp.get_fg(x, y)

    def get_bg(self, x, y):
        return self.dsp.get_bg(x, y)
