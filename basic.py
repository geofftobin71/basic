...
class Interpreter(NodeVisitor):
    ...
    def visit_StyleNode(self, node):
        ...
        op = node.op.type
        if op == TokenType.BG:
            self.vdu.bg_color = value
        elif op == TokenType.FG:
            self.vdu.fg_color = value
        elif op == TokenType.INVERSE:
            if value:
                tmp = self.vdu.fg_color
                self.vdu.fg_color = self.vdu.bg_color
                self.vdu.bg_color = tmp
            else:
                tmp = self.vdu.fg_color
                self.vdu.fg_color = self.vdu.bg_color
                self.vdu.bg_color = tmp
    ...
    def visit_InputNode(self, node):
        if self.vdu.input_ready():
            self.vdu.start_input(node.prompt)
            self.next_node = self.current_node
            return

        if self.vdu.input_editing():
            self.next_node = self.current_node
            return
    ...
    def visit_CursorNode(self, node):
        ...
        self.vdu.print_cursor_x = x
        self.vdu.print_cursor_y = y
